<?php
    ...
    if (!preg_match('/^[a-zA-Z0-9_.\s]{2,}$/', $nickname = $data['nickname'])) {
        return responseToJson("error", "Informe e -- $nickname um apelido válido.");
        
    }
    
    // Verifica se é um quantidade válido.
    if (!preg_match('/^[a-zA-Z0-9_.\s]{2,}$/', $quantid = $data['quantid'])) {
        return responseToJson("error", "Informe um $quantid quant válido.");
        
    }
    
    // Verifica se é um tipo válido.
    if (!preg_match('/^[a-zA-Z0-9_.\s]{2,}$/', $tipo = $data['tipo'])) {
        return responseToJson("error", "Informe $tipo um tipo válido.");
        
    }
    

    // Verifica se o valor da tipo é válido.
     ...
     */
    $query = "INSERT INTO donations (id_reference, nickname, quantid, tipo, email, message, status, value) VALUES (:id_reference, :nickname, :quantid, :tipo, :email, :message, :status, :value)";
    $stmt  = $pdo->prepare($query);
    
    $stmt->bindValue(':id_reference', $paymentReferenceId);
    $stmt->bindValue(':nickname',     $nickname);
    $stmt->bindValue(':quantid',      $quantid);
    ...
    */
    ...
    $informations = [
        ...
        "description"        => "{$nickname} comprou {$quantid} {$tipo}!",
    ...
    $response2 = $response1["point_of_interaction"]["transaction_data"];

    // Retorna os dados em JSON para o JavaScript.
    header('Content-Type: application/json');

    echo json_encode([
        "qr_code"        => $response2['qr_code'],        // Código copia e cola.
        "qr_code_base64" => $response2['qr_code_base64'], // Imagem do QR Code em base64.
        "external_reference" => $response1['external_reference']
    ]);
